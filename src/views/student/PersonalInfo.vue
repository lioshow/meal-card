...
export default {
  ...
  methods: {
    getInfo () {
      this.formData = JSON.parse(window.localStorage.getItem('student'))
    },
    // 打开修改弹框
    updateForm () {
    ...
    resetForm () {
      this.$confirm('确定重置密码吗？', '提示', {
        ...
        axios.put('http://localhost:8080/student/reset', this.formData).then((res) => {
          // console.log(res)
          this.loading = false
          if (res.data.code) {
      ...
      this.$refs.ruleForm.validate(async (valid) => {
        if (valid) {
          ...
          if (this.ruleForm.password !== this.ruleForm.checkPass) {
            ...
            axios.put('http://localhost:8080/student/updatePassword', this.formData).then((res) => {
              // console.log(res)
              this.$message.success(res.data.msg)
            }).finally(() => {
              this.dialogFormVisible = false
